...
ui <- fluidPage(
  ...
  sidebarPanel(width = 3,
               conditionalPanel(
                 ...
                 conditionalPanel (
                   condition = "input.data_input =='2' || input.data_input == '3'",
                   h5("To use ggplot, the data needs to be in long format. See the", actionLink("data_format", "Data Format"), " tab for more info."),
                   h5("If your data is already in long format, select \"My data is correctly formatted\"."),
                   h5("To reformat your data, choose metadata columns below. Metadata columns contains unique identifying information about each sample/observation."),
                   radioButtons("data_already_formatted", label = "How should we process your raw data?", choices = list("My data is already correctly formatted. Do not reformat." = "no", "Reformat my data from wide to long." = "yes")),
                   conditionalPanel(condition = "input.data_already_formatted == 'yes'",
                                    selectInput("dataIDs", "Select Metadata Columns:", choices = NULL, multiple = TRUE)
                   ),
                   actionButton("melt_data_button", "Submit My Data",style="background-color: #80c3ff")
                 )
               ),
               conditionalPanel(
  ...
  mainPanel(width = 6,
            tabsetPanel(
              type = "tabs",
              tabPanel("Data upload", dataTableOutput("out_table"), dataTableOutput("melted_data")),
              tabPanel("ggplot",
                       mainPanel(
  ...
  contribute to R"
              ...
              tabPanel("Data Format",
                       h3("Data Format"),
                       p("The following explanation of long and wide data is from R-Ladies Sydney's blog post on 11/28/2018."),
                       p("When you entered your data in excel, you probably entered it in wide format. Data in wide format has observations spread across several columns; each column contains data from a different condition (or combination of conditions). wide data is messy."),
                       p("In the wide example below, we have scores from participants who were tested twice (Time 1 and Time 2) after studying different lists of words under two different conditions (Condition 1 and Condition 2)."),
                       img(src = "wide_format_example.png", width = 700, height = 500),
                       p("In contrast, data in long format has all the observations in a single column and variables in separate columns."),
                       img(src = "long_format_example.png", width = 700, height = 660),
                       h3("Changing from wide to long"),
                       img(src = "wide_to_long.gif", width = 400, height = 400),
                       p("The dataframe on the left is in wide format; the performance of each baker in each spice test is listed across different columns. The dataframe on the right is in long format; all of the accuracy scores are in a single column and information about which baker and spice test each observation came from are represented in separate variables."),
                       img(src = "wide_to_long_bakers.png", width = 700, height = 370)
              ),
              id = "tabs"
            )
  ),
      ...
      tabsetPanel(
        ...
        tabPanel(
          ...
          numericInput("fig_height", "Plot height (# pixels): ",
                       value = 800),
          numericInput("fig_width", "Plot width (# pixels):", value = 1000)
        ),
        tabPanel(
          "Download",
          ...
          radioButtons("download_as", "Download File Type:", choices = list("PDF" = ".pdf", "SVG" = ".svg", "PNG" = ".png", "JPEG" = ".jpeg", "GIF" = ".gif"), selected = ".pdf"),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$upload, {
    updateSelectInput(session, 'dataIDs', choices = NULL, selected = NULL)
    values$data_melted = NULL
  })
  
  observeEvent(input$data_already_formatted, {
    if (input$data_already_formatted == TRUE) {
      updateSelectInput(session, 'dataIDs', choices = NULL, selected = NULL)
    }
  })
  ...
  df_shiny <- reactive({
    if (input$data_input == 1) {
      ...
      if (is.null(input$upload)) {
        ...
        #isolate({
        req(input$upload_delim)
        if (input$file_type == "text") {
          values$data <- read_delim(file_in$datapath,
                                    delim = input$upload_delim, quote = "\"")
        } else if (input$file_type == "Excel") {
          values$data <- read_excel(file_in$datapath)
        } else if (input$file_type == "SPSS") {
          values$data <- read_sav(file_in$datapath)
        } else if (input$file_type == "Stata") {
          values$data <- read_dta(file_in$datapath)
        } else if (input$file_type == "SAS") {
          values$data <- read_sas(file_in$datapath)
        }
        # })
      }
    } else if (input$data_input == 3) {
      if (input$data_paste == "") {
        values$data <- data.frame(x = "Copy your data into the textbox,
                             select the appropriate delimiter, and
                             press 'Submit data'")
      } else {
        values$data <- read_delim(input$data_paste,
                                  delim = input$text_delim,
                                  col_names = TRUE)
      }
    }
    
    updateSelectInput(session, 'dataIDs', choices = names(values$data))
    
    return(values$data)
  })
  
  observeEvent(input$melt_data_button, {
    if (input$data_already_formatted == "yes") {
      req(input$dataIDs)
      values$data_melted = melt(values$data, id.vars=c(input$dataIDs), variable.name="Observation", value.name = "Value")
    } else {
      values$data_melted = values$data
      updateSelectInput(session, 'dataIDs', choices = NULL, selected = NULL)
    }
  })  
  
  observeEvent(input$data_format, {
    newvalue <- "Data Format"
  ...
  output$out_table <- renderDataTable(
    df_shiny(), options = list(pageLength = 5)
  )
  
  output$melted_data <- renderDataTable(
    values$data_melted, options = list(pageLength = 5)
  )
  ...
  output$download_plot <- downloadHandler(
    filename <- function() {
      paste("Figure_ggplotGUI_", Sys.time(), input$download_as, sep = "")
    },
    content <- function(file) {
      df <- values$data_melted
      p <- eval(parse(text = string_code()))
      ggsave(file, p, width = width_download(),
             height = height_download(), units = "cm", dpi = 300)
    },
    contentType = "image" # MIME type of the image
  )
